(function() {
  ...
  angular.module("cmd.guild").controller("memberWorkloadController", memberWorkloadController);

  /** @ngInject */
  function memberWorkloadController($mdDialog, $translate, member, others, guild, Global) {
    var self = this;
      ...
      _.each(self.guild.categories, function(categories, index) {
        ...
        if (moment().isBefore(moment(self.guild.world.start).add(index, "weeks"), "day")) {
          ...
          _.each(self.member.cards, function(card) {
            if (card.done) {
              if (
                ...
                moment(card.dateLastActivity).isSame(
                  moment(self.guild.world.start).add(index, "weeks"),
                  "day"
                ) ||
                moment(card.dateLastActivity).isSame(
                  moment(self.guild.world.start)
                    .add(index, "weeks")
                    .add(6, "days"),
                  "day"
                )
          ...
          self.series[1].data.push(
            roundToDecimalPoint((count_average + count_member) / (self.others.length + 1), 1)
          );
        }
      });
    ...
    function showGraph(categories) {
      Highcharts.chart("workload__graph", {
        ...
        tooltip: {
          ...
          pointFormat:
            '<tr><td>{series.name}: </td><td><b style="padding: 0 4px;">{point.y}</b></td></tr>',
          footerFormat: "</table>",
          shared: true,
          useHTML: true,
        },
