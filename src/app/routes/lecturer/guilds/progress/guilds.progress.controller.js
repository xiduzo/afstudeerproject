(function () {
  ...
  function GuildDetailProgressController(
    ...
    Global.setRouteTitle(
      $translate.instant('TEAM') + ' ' + $translate.instant('PROGRESS').toLowerCase()
    );
      ...
      if (!guild.no_trello_settings && vm.local_trello_user) {
        TrelloApi.Authenticate().then(
          function () {
            ...
            TrelloApi.Rest('GET', 'boards/' + guild.trello_board + '/members/normal').then(
              function (response) {
                ...
                TrelloApi.Rest('GET', 'boards/' + guild.trello_board + '/cards').then(function (
                  response
                ) {
