;(function() {
  ...
  angular.module('cmd.guilds').controller('GuildDetailProgressController', GuildDetailProgressController)
  ...
  function GuildDetailProgressController(
    ...
    Global.setRouteTitle($translate.instant('TEAM') + ' ' + $translate.instant('PROGRESS').toLowerCase())
    ...
    vm.local_trello_user = localStorageService.get('trello_user') ? true : false
    ...
    Guild.getGuild($stateParams.guildUuid).then(
      function(response) {
        ...
        Global.setRouteTitle(
          $translate.instant('TEAM') + ' ' + $translate.instant('PROGRESS').toLowerCase() + ' ' + response.name
        )
        ...
        TrelloApi.Authenticate().then(
          function() {
            ...
            TrelloApi.Rest('GET', 'boards/' + guild.trello_board + '/members/normal').then(function(response) {
              ...
              TrelloApi.Rest('GET', 'boards/' + guild.trello_board + '/cards').then(function(response) {
                _.each(response, function(card) {
                  ...
                  _.each(weeks, function(week) {
                    if (
                      moment(card.created_at).isBetween(week.start, week.end, 'day') ||
    ...
    function buildGraphs(graph_data, guild) {
      $('#cards_per_week').highcharts({
        ...
        exporting: {
          filename: 'Aantal kaarten per lid per week_' + guild.name + '_' + moment().format(),
        },
      ...
      $('#cards_total').highcharts({
        chart: { type: 'pie' },
        exporting: {
          filename: 'Aantal kaarten per lid totaal_' + guild.name + '_' + moment().format(),
        },
        title: {
          text: guild.name + ': ' + $translate.instant('AMOUNT_OF_CARDS'),
        },
