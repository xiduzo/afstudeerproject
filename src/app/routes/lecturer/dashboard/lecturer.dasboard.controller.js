(function() {
  ...
  function LecturerDashboardController(
          ...
          TrelloApi.Rest("GET", "boards/" + guild.trello_board + "/cards")
            .then(function(response) {
              _.each(response, function(card) {
                if (
                  card.due &&
                  moment(card.due).isBefore(moment()) &&
                  card.idList != guild.trello_done_list
                ) {
    ...
    function getFeedbackProgression(guild) {
      var current_week =
        guild.course_duration -
        Math.floor(
          moment
            .duration(
              moment(guild.start).add(guild.course_duration, "weeks") - moment()
            )
            .asWeeks()
        );
      ...
      _.each(guild.rules, function(rule) {
        _.each(rule.endorsements, function(endorsement) {
          ...
          if (endorsement.week == current_week - 1) {
            if (
              _.findWhere(guild.members, { user_id: endorsement.endorsed_by })
            ) {
              _.findWhere(guild.members, { user_id: endorsement.endorsed_by })
                .endorsed_complete++;
            }
      ...
      _.each(guild.members, function(member) {
        member.endorsed_complete =
          (member.endorsed_complete * 100) /
          (guild.rules.length * (guild.members.length - 1)); // Substract yo-vm
      });
    }
